...
class CancelledOrdersReport extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $customers = Customer::select([
            ...
            'customers.name',
            'customers.email',
            DB::raw(value: 'COUNT(CASE WHEN customer_order.status = "Cancelled" and customer_order.created_at >= "'.$this->from.'" and customer_order.created_at <= "'.$this->to.'" then customer_order.cancellation_reason_id end) as total'),
        ])
        ...
        return view('livewire.report.cancelled-orders-report', [
            'customers' => $customers
        ]);
    }
}
