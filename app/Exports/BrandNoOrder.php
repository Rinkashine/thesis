...
class BrandNoOrder implements FromCollection, ShouldAutoSize, WithHeadings, WithMapping,  WithStyles, WithEvents, WithColumnWidths, WithDrawings
{
    protected $sorting;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                // dd($this->sort);
                $event->sheet->setCellValue('A2','Orders by Brand Report');
    ...
    public function __construct($sorting, $startdate, $enddate)
    {
        ...
        if ($this->sorting == 'brand_name_asc') {
            $this->sort = 'Brand Name (A-Z)';
            $this->column_name = 'name';
            $this->order_name = 'asc';
        } elseif ($this->sorting == 'brand_name_desc') {
            $this->sort = 'Brand Name (Z-A)';
            $this->column_name = 'name';
    ...
    public function headings(): array
    {
        return [
            ...
            [],
            [],
            ['Brand Name', 'Total Ordered Products',]
        ];
    }
}
