...
class RejectedOrdersExport implements FromCollection,WithHeadings,ShouldAutoSize,WithMapping,WithStyles, WithEvents, WithColumnWidths, WithDrawings
{
    protected $sorting;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                // dd($this->sort);
                $event->sheet->setCellValue('A2','Customer Total Rejected Orders Report');
    ...
    public function __construct($sorting, $startdate, $enddate)
    {
        ...
        if($this->sorting == 'customer_name_asc'){
            ...
            $this->sort = 'Total Rejections (Low-High)';
            $this->column_name = 'total';
            $this->order_name = "asc";
        }elseif($this->sorting == 'total_spent_desc'){
            $this->sort = 'Total Rejections (High-Low)';
            $this->column_name = 'total';
            $this->order_name = 'desc';
        }
        // dd($this->sort);
    ...
    */
    public function collection()
    {
        ...
        return Customer::select([
            ...
            'customers.name',
            'customers.email',
            DB::raw(value: 'COUNT(CASE WHEN customer_order.status = "Rejected" and customer_order.created_at >= "'.$this->startdate.'" and customer_order.created_at <= "'.$this->enddate.'" then customer_order.rejected_reason end) as total'),        
    ...
    public function headings(): array
    {
        return [
            ...
            [],
            [],
            ['Customer Name','Email', 'Rejections',]
        ];
    }
}
