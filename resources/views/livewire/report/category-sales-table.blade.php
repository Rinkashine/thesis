                    ...
                    <div class="dropdown w-1/2 sm:w-auto">
                        ...
                        <div class="dropdown-menu w-40">
                            <ul class="dropdown-content">
                                <li>
                                    <a href="{{Route('exportSalesCategoryEXCEL',['startdate'=>$from,'enddate'=>$to])}}" class="dropdown-item"> <i class="fa-solid fa-file-excel mr-1"></i> Export Excel </a>
                                </li>
                                <li>
                                    <a  href="{{Route('exportSalesCategoryCSV',['startdate'=>$from,'enddate'=>$to])}}" class="dropdown-item"> <i class="fa-solid fa-file-csv mr-1"></i>  Export CSV </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                        ...
                        <tbody>
                            @foreach ($categories as $category )
                                <tr>
                                    ...
                                    <td class="whitespace-nowrap text-center">{{$category->order_total}}</td>
                                    <td class="whitespace-nowrap text-center">{{$category->order_quantity}}</td>
                                    <td class="whitespace-nowrap text-center">{{$category->total_sales}}</td>
                                </tr>
                            @endforeach
                            {{-- @foreach ($nonbuyer as $category )
                                <tr>
                                    <td class="whitespace-nowrap ">{{$category->name}}</td>
                                    <td class="whitespace-nowrap text-center">{{$category->email}}</td>
